  ...
  try {
    ...
    if (err instanceof NetworkError) {
      if (err.statusCode === 401)
        throw new AppError("Unauthorized", "AUTH_ERROR");
...
Code that does `catch (err) { console.error(err.message) }` crashes in production when someone `throw "a string"` — because strings don't have `.message`. `useUnknownInCatchVariables` forces you to handle this class of bug at compile time. Combined with the `formatError` utility, catch blocks become genuinely safe.
